...
for e in range(0,len(split_ver)):
    if len(split_ver[e])==3:
        print(split_ver[e],total_len%2)
        thes.append([split_ver[e],total_len%2])
    total_len+=len(split_ver[e])

tempthes=copy.deepcopy(thes)


def freq_key(item):
    print(tempthes.count(item))
    return tempthes.count(item)


print(thes)
thes.sort(key=freq_key,reverse=True)
print(thes)
